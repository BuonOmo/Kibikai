...
public class IASoldier extends IAUnite {
    ...
    public int calculateStaite() {
        ...
        for (SimpleUnit i : simpleUnitPlyaerInZone3) {
            ...
            for (Soldier j : soldierPlyaerInZone3) {
                if (!i.isCloseTo(j, Finals.ATTACK_RANGE * 2))
                    isIsolate = false;
            }
            if (isIsolate) {
                state = 8 + state;
        ...
        if (nbComputerSoldiersCloseToBat > 0) {
            if (nbPlayerSoldiersCloseToBat < nbComputerSoldiersCloseToBat)
                state = 16 + state;
            else
                state = 32 + state;
        }
         ...
         */
        if (soldierPlyaerInZone3.size() * 0.8 > soldierComputerInZone3.size())
            state = 48 + state;
        else if (soldierPlyaerInZone3.size() * 1.5 > soldierComputerInZone3.size())
            state = 96 + state;


        if (unitGroup.numberUnit() < (double) Finals.NUMBER_MAX_OF_SOLDIER * 0.2)
            state = 144 + state;
        else if (unitGroup.numberUnit() < (double) Finals.NUMBER_MAX_OF_SOLDIER * 0.08)
            state = 288 + state;
        return state;
    }
    ...
    public int chooseStrategy(int staite) {
        ...
        /*
         * pr�cotion pris puisqu'il y a ue des soucie a de d�passement d'indice mais il dvrais t�re apr�sent r�solsuts
         */
        if (staite > 431) {
        ...
        for (int i = 0; i < 6; i++) {
            if (Rdm < Math.exp(IA.qIASoldier[staite][i]))
                return i + 1;
